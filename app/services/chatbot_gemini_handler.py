...
def handle_java_chatbot_request(
    ...
    try:
        ...
        # 1차 파싱: AI 응답 텍스트를 파이썬 딕셔너리로 변환
        try:
            ai_data_dict = json.loads(ai_response_text)
        except json.JSONDecodeError as e:
            print(f"1차 JSON 파싱 실패: {e}\nRaw AI Text: {ai_response_text}")
            return plan_actions.simple_message(f"AI 응답이 JSON 형식이 아닙니다: {ai_response_text}")

        # 중첩된 JSON 문자열 처리 로직 (target 강제 파싱)
